...
// void SimpleKNN::knn(int P, float3* points, float* meanDists)
// {
//     // Get the device from the pointer attributes
//     cudaPointerAttributes attributes;
//     cudaPointerGetAttributes(&attributes, points);
//     int device_id = attributes.device;
...
//     // Create a custom allocator for Thrust
//     using thrust_allocator = thrust::cuda::allocator<char>;
//     thrust_allocator alloc;

//     // Use CUB to find the minimum
//     cub::DeviceReduce::Reduce(nullptr, temp_storage_bytes, points, result, P, CustomMin(), init);
//     thrust::device_vector<char, thrust_allocator> temp_storage(temp_storage_bytes, alloc);

//     cub::DeviceReduce::Reduce(temp_storage.data().get(), temp_storage_bytes, points, result, P, CustomMin(), init);
//     cudaMemcpy(&minn, result, sizeof(float3), cudaMemcpyDeviceToHost);

//     // Use CUB to find the maximum
//     cub::DeviceReduce::Reduce(temp_storage.data().get(), temp_storage_bytes, points, result, P, CustomMax(), init);
//     cudaMemcpy(&maxx, result, sizeof(float3), cudaMemcpyDeviceToHost);

//     // Create device vectors with custom allocator
//     thrust::device_vector<uint32_t, thrust_allocator> morton(P, alloc);
//     thrust::device_vector<uint32_t, thrust_allocator> morton_sorted(P, alloc);

//     // Launch kernel
//     coord2Morton<<<(P + 255) / 256, 256>>>(P, points, minn, maxx, morton.data().get());

//     thrust::device_vector<uint32_t, thrust_allocator> indices(P, alloc);
//     thrust::sequence(indices.begin(), indices.end());

//     thrust::device_vector<uint32_t, thrust_allocator> indices_sorted(P, alloc);

//     // Sort pairs using CUB
//     cub::DeviceRadixSort::SortPairs(nullptr, temp_storage_bytes, morton.data().get(), morton_sorted.data().get(),
//                                     indices.data().get(), indices_sorted.data().get(), P);
//     temp_storage.resize(temp_storage_bytes);

//     cub::DeviceRadixSort::SortPairs(temp_storage.data().get(), temp_storage_bytes, morton.data().get(),
//                                     morton_sorted.data().get(), indices.data().get(), indices_sorted.data().get(), P);

//     uint32_t num_boxes = (P + BOX_SIZE - 1) / BOX_SIZE;

//     thrust::device_vector<MinMax, thrust_allocator> boxes(num_boxes, alloc);
